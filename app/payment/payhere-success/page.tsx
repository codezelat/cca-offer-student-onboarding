...
export default async function PayHereSuccessPage({ searchParams }: Props) {
  ...
  const whatsappGroups = Array.from(
    new Map(
      allRecords
        .map((record) => {
          const link = getBootcampWhatsappLink(record.selected_diploma);
          return link
            ? [record.selected_diploma, link]
            : null;
        })
      ...
      <div className="flex flex-col items-center mx-auto max-w-3xl space-y-12 py-8 px-2">
        ...
        <div className="w-full flex flex-col space-y-10">
          <div className="hidden print:block print:mb-8 print:border-b-2 print:border-neutral-900 print:pb-4">
            <h1 className="text-2xl font-bold uppercase tracking-widest text-neutral-900">
          ...
          >
            <div
              className={`mb-8 flex h-16 w-16 items-center justify-center rounded-3xl ${completed ? "bg-emerald-500 shadow-emerald-200" : "bg-amber-500 shadow-amber-200"} text-white shadow-xl animate-in zoom-in duration-500 no-print`}
            >
              {completed ? (
                <svg
                  className="h-8 w-8"
            ...
            <h1
              className={`text-4xl font-bold tracking-tight ${completed ? "text-emerald-950" : "text-amber-950"}`}
            >
              {completed
            ...
            <p
              className={`mt-4 text-lg font-medium leading-relaxed ${completed ? "text-emerald-900" : "text-amber-900"}`}
            >
              {completed
            ...
            {completed ? (
              ...
              <div className="mt-8 flex items-center gap-3 rounded-2xl bg-white/60 p-4 border border-amber-200/50 w-fit no-print">
                ...
                <span className="text-sm font-semibold text-amber-900">
                  {publicCopy.paymentSuccess.processingRefresh.replace(
                    "5s...",
                    "Checking status...",
                  )}
                </span>
          ...
          <section className="rounded-[2rem] border border-neutral-200 bg-white p-6 sm:p-10">
            ...
            <div
              className={`mt-8 rounded-[1.5rem] border ${completed ? "border-emerald-200 bg-white" : "border-neutral-200 bg-neutral-50"} p-8 shadow-sm`}
            >
              ...
              <div className="flex flex-wrap gap-3">
                {allRecords.map((r) => (
                  <div
                    ...
                    className="rounded-xl border border-neutral-100 bg-neutral-900 px-5 py-3 text-lg font-black tracking-tight text-white shadow-lg shadow-neutral-900/10 print:bg-neutral-100 print:text-neutral-900"
                  >
                    {r.registration_id}
                  </div>
                ))}
              ...
              <PaymentLine
                ...
                value={formatCurrency(
                  student.amount_paid?.toString() ??
                    (3000 * allRecords.length).toString(),
                )}
              />
              <PaymentLine
            ...
            {completed ? (
              <>
                ...
                <div className="mt-8 grid gap-4 sm:grid-cols-2">
                  {publicCopy.paymentSuccess.nextSteps.map(
                    ([title, body], idx) => (
                      <div
                        key={title}
          ...
          {completed && whatsappGroups.length > 0 ? (
            <section className="rounded-[2rem] border border-emerald-200 bg-emerald-50 p-6 sm:p-10">
              ...
              <p className="mt-3 text-sm leading-6 text-emerald-900">
                {whatsappGroups.length === 1
                  ...
                  : "Join each WhatsApp group for the program(s) you completed payment for."}
              </p>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {whatsappGroups.map(({ bootcamp, link }) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex justify-center items-center gap-3 rounded-xl bg-[#25D366] px-6 py-5 text-center text-base font-bold text-white transition-all hover:bg-[#20bd5a] hover:shadow-xl hover:shadow-[#25D366]/20 active:scale-[0.98] w-full"
                  >
                    <svg
...
function PaymentLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-neutral-100 bg-white p-4 transition-all hover:border-neutral-200">
      ...
      <dd className="mt-1 text-sm font-semibold text-neutral-900 line-clamp-1">
        {value}
      </dd>
    </div>
  );
