...
export async function GET(
  ...
  page.drawRectangle({
    x: 50,
    y: currentY - ((bootcampNames.length + 1) * 25),
    width: 495,
  ...
  bootcampNames.forEach((name) => {
    page.drawText(`${name} Registration`, { x: 70, y: rowY, size: 10, font });
    page.drawText("Rs. 3,000.00", {
      x: 450,
      y: rowY,
      size: 10,
      font,
  ...
  page.drawText(`Rs. ${(3000 * bootcampNames.length).toLocaleString()}.00`, {
    x: 440,
    y: rowY + 12,
    size: 11,
    font: boldFont,
  ...
  page.drawText("PAYMENT SUBMITTED", {
    x: 70,
    y: currentY + 15,
    size: 10,
    font: boldFont,
    color: rgb(0.04, 0.3, 0.15),
  });
  page.drawText(
    `via ${student.payment_method === "online" ? "PayHere Secure Gateway" : "Bank Slip Verification"}`,
    {
      x: 180,
      y: currentY + 15,
      size: 9,
